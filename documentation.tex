...
\begin{abstract}
This document describes the LiquidCan protocol, a CAN-based communication protocol
for distributed embedded systems. The protocol defines message types, data structures,
and communication patterns for node information exchange, status reporting, parameter
management, and field group updates.
\end{abstract}
...
\section{CAN Identifier Scheme}
The CAN ID is composed of 11 bits with the following structure.
...
See \nameref{messageTypes} for a detailed description of message types and their numeric values.
...
As soon as a node comes online (or when it receives a nodeInfoReq), it sends out a nodeInfoRes.
...
the firmware and LiquidCan hashes. The central server registers the new node and waits for field registration messages. From this point on the 
node is allowed to send/receive messages on the bus.

\section{Field Registration \& Management}
...
\paragraph{}
After initializing registration, a node sends out field registration messages, one for each 
...
\paragraph{}
Next, the node sends FieldGroupDefinitions. Variables and parameters cannot be mixed together in one FieldGroupDefinition.
The FieldGroupDefinitions define the byte order of future FieldGroupUpdates. The node sends a group ID to identify the group and a list of field IDs. The order of the 
field IDs must be the same as the order of fields in future FieldGroupUpdates. The node must ensure that all of the fields defined can fit into a FieldGroupUpdate.


\subsection{Regular Operation}
\subsubsection{Field Updates}
During regular operation, each node sends FieldGroupUpdates at a defined interval. 
...
Other bus members can send a parameter set request, which includes the field ID of the parameter and the new value.
Once the node receives the request, it changes the internal parameter value and responds with a parameter set response containing the 
...
When a parameter is internally modified through some automated system, the updated value must be sent as a parameter set request to the server.
\todo{Consider if this could lead to some problems -> Configurable?}
\subsubsection{Parameter Locking}

A parameter can optionally be locked and later unlocked through a parameter lock request.
...
\paragraph{}
\subsubsection{Requesting Field Data}
A field can be accessed through a field get request, which contains the field ID.
Nodes respond with a field get response, containing the field ID and the value of the field.

\section{Heartbeats}
Heartbeats ensure that the system does not reach a state where it is still dangerous to physically handle but 
not accessible through CAN messages. The heartbeat request (sent from the server) contains a continuously increasing counter.
The value of the counter is unique to each Node. In the case that Nodes do not receive heartbeat requests the node will default
into a safe state. Similary the server takes note of any unresponsive nodes.

\section{Status Messages}
Node may send optional status messages to the server.
Currently the 3 status message types are: info, warning, error
Each message contains a null terminated string with a status message.
\section{Message Types}\label{messageTypes}
...
0 & \texttt{node\_info\_req} \label{nodeInfoReq} & No payload & Request node information \\
1 & \texttt{node\_info\_res}\label{nodeInfoRes} & NodeInfoRes & Response with node capabilities and identification \\
\midrule
\multicolumn{4}{l}{\textit{Status Messages}} \\
2 & \texttt{info\_status} & Status & Informational status message \\
3 & \texttt{warning\_status} & Status & Warning status message \\
4 & \texttt{error\_status} & Status & Error status message \\
\midrule
\multicolumn{4}{l}{\textit{Field Registration}} \\
5 & \texttt{variable\_registration} & FieldRegistration & Register a variable field \\
6 & \texttt{parameter\_registration} & FieldRegistration & Register a parameter field \\
\midrule
\multicolumn{4}{l}{\textit{Field Group Management}} \\
7 & \texttt{field\_group\_definition} & FieldGroupDefinition & Define a group of fields for batch updates \\
8 & \texttt{field\_group\_update} & FieldGroupUpdate & Update values for a field group \\
\midrule
\multicolumn{4}{l}{\textit{Heartbeat}} \\
9 & \texttt{heartbeat\_req} & HeartBeat & Heartbeat request \\
10 & \texttt{heartbeat\_res} & HeartBeat & Heartbeat response \\
\midrule
\multicolumn{4}{l}{\textit{Parameter Management}} \\
11 & \texttt{parameter\_set\_req} & ParameterSetReq & Request to set a parameter value \\
12 & \texttt{parameter\_set\_res} & ParameterSetRes & Response with confirmed parameter value \\
13 & \texttt{parameter\_set\_lock\_req} & ParameterSetLock & Request to lock a parameter \\
14 & \texttt{parameter\_set\_lock\_res} & ParameterSetLock & Response confirming parameter lock \\
\midrule
\multicolumn{4}{l}{\textit{Field Access}} \\
15 & \texttt{field\_get\_req} & FieldGetReq & Request field value \\
16 & \texttt{field\_get\_res} & FieldGetRes & Response with field value \\
\bottomrule
\end{longtable}
\end{center}
...
\bitbox{1}{var\_cnt} & \bitbox{1}{par\_cnt} & \bitbox{1}{ func\_cnt} & 
\bitbox{4}{firmware\_hash} &
& &  \bitbox{1}{liquid\_hash  }
\end{leftwordgroup}\\

\begin{leftwordgroup} {1}
\bitbox{3}{liquid\_hash} \bitbox[lrt]{5}{  }
\end{leftwordgroup}\\
...
\subsection{FieldRegistration}
Registration information for a variable or parameter field.
The DataType here refers to the DataType Enum value.
...
\subsection{FieldGroupDefinition}
...
\textbf{Note:} The values are packed according to the data types of the fields
defined in the corresponding FieldGroupDefinition, ordered by the field\_ids array.
...
\section{Versioning and Extension Mechanisms}
% TODO: Describe:
% - Version tracking using firmware_hash and liquidcan_hash
% - Protocol versioning strategy
% - Backwards compatibility rules
% - How to handle unknown message types
This protocol uses semantiv versioning.
See \href{semver.org}{https://semver.org/} for a detailed description. A minor update version would for example be adding a new datatype or a new message type. Every update of the document must trigger an update of the liquidCAN repo, containing the rust and c code for each implementation. The updated repo must be reflected in the liquid\_hash version number used in the firmware/server.

\section{How to use this documentation}
\begin{enumerate}
    \item Implement heartbeat mechanism for node monitoring.
    \item Fill in the TODO sections with project-specific details.
    \item Test cross-compiler compatibility of struct layouts.
    \item Add application-specific message types as needed.
\end{enumerate}
\end{document}