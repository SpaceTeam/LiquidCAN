...
device\_name & $\leq$53 & Human-readable device name (null-terminated) \\
\midrule
\textbf{Total} & \textbf{11--63} & \\
\bottomrule
\end{tabular}
\end{center}
...
Field & Bytes & Description \\
\midrule
msg & $\leq$63 & Status message text (null-terminated) \\
\bottomrule
\end{tabular}
\end{center}
...
field\_name & $\leq$61 & Human-readable field name (null-terminated) \\
\midrule
\textbf{Total} & \textbf{3--63} & \\
\bottomrule
\end{tabular}
\end{center}
...
field\_ids & $\leq$62 & Array of field IDs in this group (terminated by end of frame) \\
\midrule
\textbf{Total} & \textbf{2--63} & \\
\bottomrule
\end{tabular}
\end{center}
...
values & $\leq$62 & Packed values of all telemetry values in the group \\
\midrule
\textbf{Total} & \textbf{2--63} & \\
\bottomrule
\end{tabular}
...
value & 1--4 & New value (size depends on parameter datatype) \\
\midrule
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t value[]; // 1-4 bytes, flexible array member
} parameter_set_req_t;
\end{lstlisting}

\subsection{FieldStatus}\label{subsec:FieldStatus}
...
\midrule
0 & \texttt{Ok} & Field exists and operation was successful \\
1 & \texttt{NotFound} & Field does not exist \\
\bottomrule
\end{tabular}
\end{center}
...
value & 1--4 & Confirmed value after set operation (size depends on datatype) \\
\midrule
\textbf{Total} & \textbf{3--6} & \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Note:} If the parameter is not found (field\_status = NotFound), the value field should contain the originally requested value from the ParameterSetReq message, and the status field should indicate InvalidParameterID.
...
\midrule
field\_id & 1 & Field identifier \\
field\_status & 1 & Field existence status (FieldStatus enum) \\
value & 1--4 & Field value (size depends on field datatype) \\
\midrule
\textbf{Total} & \textbf{3--6} & \\
\bottomrule
\end{tabular}
...
\textbf{Note:} If the field is not found (field\_status = NotFound), the field\_id should still contain the requested field ID from the FieldGetReq message, and the value field content is undefined.
...
Field & Bytes & Description \\
\midrule
field\_name & $\leq$61 & Field Name (null-terminated) \\
\midrule
\textbf{Total} & \textbf{2--61} & \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Note:} If the field is not found (field\_status = NotFound), the field\_id should be set to 0 (reserved invalid ID) and field\_type is undefined.
...
\subsection{ParameterSetLockConfirmation}\label{struct:ParameterSetLockConfirmation}
...
parameter\_id & 1 & Parameter identifier to lock \\
parameter\_lock & 1 & Lock status(0=unlocked, 1=locked) \\
field\_status & 1 & Field status (FieldStatus enum) \\
\midrule
\textbf{Total} & \textbf{3} & \\
\bottomrule
\end{tabular}
...
\textbf{Note:} If the parameter is not found (field\_status = NotFound), the parameter\_id should still contain the requested parameter ID from the ParameterSetLockReq message.

% TODO: Describe parameter locking mechanism and use cases

\begin{lstlisting}[caption={ParameterSetLockConfirmation struct}]
typedef struct __attribute__((packed)) {
    uint8_t parameter_id;
    uint8_t lock_status;
    uint8_t field_status;
} parameter_set_lock__confirmation_t;
\end{lstlisting}
